...
def file_processor(df, lookup):
    ...
    #highlight_columns = ['Travel', 'Service', 'Disposal', 'Pre/Post', 'Target Clock Hours']

    #def styling(df2, columns):

        #styles = []
        #for column in df2.columns:
            #style = 'background-color: yellow' if column in columns else ''
            #styles.append(style)
        #return styles
        
    
        #for i, row in df2.iterrows():
            #if df2['Variance to Target'] > 0:
    ...
    #styled = df2.style.apply(styling, columns=highlight_columns, axis=1)
